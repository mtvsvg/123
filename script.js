...
function parseLine(line) {
    ...
    while (i < words.length) {
        ...
        if (isName && !isPosition && nameParts.length < 3) { nameParts.push(w); i++; } else { positionParts.push(w); i++; }
    }
    if (nameParts.length < 2) {
        const firstThree = words.slice(0, Math.min(3, words.length));
        if (firstThree.length >= 2) { nameParts = firstThree; positionParts = words.slice(firstThree.length); }
    }
...
function initTrainingPage() {
    ...
    document.getElementById('orgSelect').addEventListener('change', function() {
        if (this.value) { selectOrg(parseInt(this.value)); } else { currentOrgId = null; localStorage.removeItem('currentOrgId'); }
    });
    ...
    document.getElementById('staffImportBtn').addEventListener('click', function() {
        ...
        input.onchange = function(e) {
            ...
            reader.onload = function(event) {
                ...
                if (employees.length === 0) { alert('❌ Не удалось распознать данные. Проверьте формат файла.'); document.body.removeChild(input); return; }
                const currentStaff = getStaff();
                employees.forEach(emp => {
                    if (!currentStaff.some(e => e.last_name === emp.last_name && e.first_name === emp.first_name && e.snils === emp.snils)) currentStaff.push(emp);
                });
    ...
    document.getElementById('addSelectedBtn').addEventListener('click', function() {
        ...
        selected.forEach(emp => {
            if (!currentProtocol.some(e => e.last_name === emp.last_name && e.first_name === emp.first_name && e.snils === emp.snils)) currentProtocol.push(emp);
        });
    ...
    document.getElementById('generateBtn').addEventListener('click', function() {
        ...
        if (selectedPrograms.length === 0) { alert('Выберите хотя бы одну программу обучения'); return; }
